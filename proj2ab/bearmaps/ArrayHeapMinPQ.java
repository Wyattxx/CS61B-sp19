...
public class ArrayHeapMinPQ<T> implements ExtrinsicMinPQ<T> {
    ...
    private void swap(int index1, int index2) {
        ...
        myItemMapIndex.put(node2.myItem, index1);
        myItemMapIndex.put(node1.myItem, index2);
    }
...
//       }
    ...
    public boolean contains(T item) {
        return myItemMapIndex.containsKey(item);
    }
    ...
    public T removeSmallest() {
        ...
        swap(1, size);
        contents[size] = null; // no loitering
        myItemMapIndex.remove(min);
        size -= 1;
        if (size > 0) {
            sink(1);
        }
        if ((double) size / contents.length < 0.25) {
            resize(contents.length / 2);
        }
        return min;
    }
    ...
    public void changePriority(T item, double priority) {
        if (size == 0 || !myItemMapIndex.containsKey(item)){
            throw new NoSuchElementException();
        }
        int i = myItemMapIndex.get(item);
